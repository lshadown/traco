...
def GetProperSET_II(l, _SYM, sym_exvars, par_vars, stuff, vars, BLOCK):
    SET_II = "[" + _SYM[:-1] + "] -> {[" + ",".join(vars) + ",v] : "
    for i in range(0, l):
        SET_II = SET_II + sym_exvars[i] + " >= 0 and " + par_vars[i] + " <= " + stuff[i]['ub'] + " and "
        SET_II = SET_II+ Constr_Wbloku(vars, sym_exvars, stuff, BLOCK, i, par_vars[i], i) 
    SET_II = SET_II+ "1=1 }" 
    return isl.Set(SET_II)
...
def tile(plik, block):
    ...
    isl_rel_invert = isl_rel.fixed_power(-1).coalesce()
    isl_rel_invert_closure = isl_rel_invert.transitive_closure()[0]
    ...
    for i in range(0, len(instrukcje)):
        ...
        BLGT = MakeBLTandBGT_v2(_SYM, vars, sym_exvars, par_vars, varsprim, exvars, stuff, BLOCK, instrukcje[i], instrukcje[i])   # porownaj zagniezdzenia instrukcji
        isltmp = isl.Set(str(BLGT[0]).replace("_BLT := ", ""))
        isl_TILE_LT.append(isltmp)
        isltmp = isl.Set(str(BLGT[1]).replace("_BGT := ", ""))  
        isl_TILE_GT.append(isltmp)
        ...
        for j in range(0, i):
            isl_TILEprim[i] = isl_TILEprim[i].subtract(Project(isl_TILEprim[j], sym_exvars)).coalesce()

        Z = isl_TILE[i].apply(isl_rel_invert_closure).coalesce()
        for j in range(i+1, len(instrukcje)):
            Z1 = Z.intersect(isl_IT[j])
            _min = min(instrukcje[i]['nest'], instrukcje[j]['nest'])
            tmp_setii = GetProperSET_II(_min, _SYM, sym_exvars, par_vars, stuff, vars, BLOCK)
            Z1 = Z1.intersect(tmp_setii).coalesce()
            isl_TILEprim[i] = Z1.union(isl_TILEprim[i]).coalesce()

        isl_TILEbis.append(Project(isl_TILEprim[i].apply(Rapply).coalesce(), sym_exvars).coalesce())
    

    loop = codegen.tile_gen(vars, sym_exvars, symb, isl_TILEbis, instrukcje, "", [])
