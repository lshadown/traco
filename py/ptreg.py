...
loop = '''for( c1 = 0; c1 <= floord(N - 1, 7); c1 += 1)
  for( c3 = 0; c3 <= min(floord(N - 2, 19), (7 * c1 + 5) / 19); c3 += 1)
    for( c4 = 0; c4 <= 1; c4 += 1) {
      if (c4 == 1) {
        for( c7 = max(-N + 7 * c1 + 1, -N + 19 * c3 + 2); c7 <= min(0, -N + 7 * c1 + 7); c7 += 1)
        ...
        for( c5 = 0; c5 <= 19 * c3 / 49; c5 += 1)
          for( c7 = max(-N + 7 * c1 + 1, -N + 19 * c3 + 2); c7 <= min(0, -N + 7 * c1 + 7); c7 += 1) {
            if (N + c7 >= 7 * c1 + 2) {
              ...
              for( c9 = N - 7 * c1 + 19 * c3; c9 <= min(N - 1, N - 7 * c1 + 19 * c3 + 18); c9 += 1)
                for( c11 = 49 * c5; c11 <= min(19 * c3, 49 * c5 + 48); c11 += 1)
                  S[(N-7*c1-1)][c9] = S[(N-7*c1-1)][c11+(N-7*c1-1)] + S[c11+(N-7*c1-1)+1][c9] + S[(N-7*c1-1)][c9];
          }
    }
'''
