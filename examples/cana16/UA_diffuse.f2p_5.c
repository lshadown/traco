...
#pragma omp parallel for
      for (c2 = 0; c2 <= min(floord(N2 - 1, 16), floord(N4 - 1, 16)); c2 += 1)
        for (c3 = 0; c3 <= min(floord(N3 - 1, 16), (N2 - 1) / 16); c3 += 1)
          for (c4 = 0; c4 <= min((N4 - 1) / 16, (N3 - 1) / 16); c4 += 1)
            for (c5 = 16 * c0 + 1; c5 <= min(16 * c0 + 16, N1); c5 += 1)
              for (c6 = 16 * c3 + 1; c6 <= min(N3, 16 * c3 + 16); c6 += 1)
                for (c7 = 16 * c4 + 1; c7 <= min(16 * c4 + 16, N4); c7 += 1)
          ...
          for (c4 = 0; c4 <= floord(N4 - 1, 16); c4 += 1)
            for (c5 = 16 * c0 + 1; c5 <= min(16 * c0 + 16, N1); c5 += 1)
              for (c6 = 16 * c2 + 1; c6 <= min(16 * c2 + 16, N2); c6 += 1)
                for (c7 = 16 * c3 + 1; c7 <= min(16 * c3 + 16, N3); c7 += 1)
                  for (c8 = 16 * c4 + 1; c8 <= min(16 * c4 + 16, N4); c8 += 1)
                    r[c8][c7][c6]=r[c8][c7][c6]+u[c8][c7][c5]*wdtdr[c5][c6];
    } else
#pragma omp parallel for
      for (c2 = 0; c2 <= floord(N2 - 1, 16); c2 += 1)
        for (c3 = 0; c3 <= floord(N3 - 1, 16); c3 += 1)
