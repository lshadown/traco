    ...
    for (c4 = 2; c4 <= 3; c4 += 1) {
      if (c4 == 3) {
        for (c7 = 32 * c1; c7 <= min(N - 1, 32 * c1 + 31); c7 += 1)
          for (c9 = max(1, 32 * c3); c9 <= min(N - 1, 32 * c3 + 31); c9 += 1)
            for (c10 = max(max(2, 32 * c3 - c9 + 3), -c9 + 4); c10 <= 3; c10 += 1) {
              if (c10 == 3) {
                B[c9][c7]=B[c9][c7]/L[c9][c9];
               // z++;
              } else
                for (c11 = max(1, 32 * c3); c11 < c9; c11 += 1){
                  B[c9][c7]=B[c9][c7]-L[c9][c11]*B[c11][c7];
               //  z++;
                }
            }
      } else
        for (c5 = 0; c5 <= min(c3, floord(N - 2, 32)); c5 += 1)
          for (c7 = 32 * c1; c7 <= min(N - 1, 32 * c1 + 31); c7 += 1)
